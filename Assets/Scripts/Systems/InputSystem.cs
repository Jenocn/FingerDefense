...
namespace Game.Systems {
	...
	public static class InputSystem {
		...
		private static List<System.Tuple<string, float, float, int>> _axisToButtonBinds = new List<System.Tuple<string, float, float, int>>();
		private static HashSet<int> _axisToButtonStateSet = new HashSet<int>();
		...
		public static void Update() {
			...
			foreach (var item in _axisToButtonBinds) {
				var value = Input.GetAxis(item.Item1);
				bool bEnter = (value >= item.Item2) && (value <= item.Item3);
				bool bContains = _axisToButtonStateSet.Contains(item.Item4);
				bool bDown = bEnter && !bContains;
				bool bUp = !bEnter && bContains;
				GetButtonValue(item.Item4).OrState(bDown, bUp, bEnter);
				if (bDown) {
					_axisToButtonStateSet.Add(item.Item4);
				} else if (bUp) {
					_axisToButtonStateSet.Remove(item.Item4);
				}
			}
		}
		...
		/// <summary>
		/// 添加摇杆模拟按键绑定,当触发摇杆值的范围时触发
		/// </summary>
		/// <param name="axisName">摇杆事件名</param>
		/// <param name="min">最小值</param>
		/// <param name="max">最大值</param>
		/// <param name="command">命令</param>
		public static ButtonValue AddButtonBind(string axisName, float min, float max, int command) {
			_axisToButtonBinds.Add(new System.Tuple<string, float, float, int>(axisName, min, max, command));
			return GetButtonValue(command);
		}
		...
		/// 删除绑定
		/// </summary>
		public static void RemoveBind(string axisName, float min, float max, int command) {
			for (int i = _axisToButtonBinds.Count - 1; i >= 0; --i) {
				var item = _axisToButtonBinds[i];
				if ((item.Item1 == axisName) && (item.Item2 == min) && (item.Item3 == max) && (item.Item4 == command)) {
					_axisToButtonBinds.RemoveAt(i);
				}
			}
		}
		...
		public static ButtonValue GetButtonValue(int command) {
			if (!_commands.TryGetValue(command, out var ret)) {
				ret = new ButtonValue();
		...
		public static AxisValue GetAxisValue(int axis) {
			if (!_axisDict.TryGetValue(axis, out var ret)) {
				ret = new AxisValue();
